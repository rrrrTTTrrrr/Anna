//  ***********************************************************   TEMPLATE OR INLINE STATIC METHODS IMPLEMENTATION   ************************************************************
		...
		inline typename ProvideTypeOnlyIfAssertionIsTrue< TypeCheck_Integer< NumericType >::Result() == true, Tuple< Boolean, NumericType > >::Type Root::Calculate( NumericType radicand, NumericType degree )
		{
			...
			Assert_MACRO( ( radicand >= 0 ), "The given radicand should not be negative" );
			Assert_MACRO( ( degree >= 0 ), "The given degree should not be negative" );
			...
			ReturnValueIfExpressionIsTrue_MACRO( ( radicand == 0 ), root )

			// Set the root to 1
			root.template Entry< 1 >() = 1;

			// Check if the radicand is 1
			ReturnValueIfExpressionIsTrue_MACRO( ( radicand == 1 ), root )
			...
			while ( true )
			{
				...
				ReturnValueIfExpressionIsTrue_MACRO( ( radicand == numberRaisedByThePowerOfTheDegree ), root )
		...
		inline typename ProvideTypeOnlyIfAssertionIsTrue< TypeCheck_FloatingPoint< NumericType >::Result() == true, NumericType >::Type Root::Calculate( NumericType radicand, NumericType degree, NumericType maximumDifferenceBetweenRootCalculatedRaisedByThePowerOfTheDegreeAndRadicand )
		{
			...
			Assert_MACRO( ( radicand >= 0 ), "The given radicand should not be negative" );
			Assert_MACRO( ( degree >= 0 ), "The given degree should not be negative" );
			Assert_MACRO( ( maximumDifferenceBetweenRootCalculatedRaisedByThePowerOfTheDegreeAndRadicand < 1 ), "The given difference between the root raised to the power of the degree should be less then 1" );
			...
			for ( NumericType incrementSize = 1 ; incrementSize > 0.0000000000000000000000000000001 ; incrementSize /= 10 )
			{
				...
				while ( true )
				{
					...
					// Check if the result match the radican, which means the root was found
					ReturnValueIfExpressionIsTrue_MACRO( ( AbsoluteValue::Calculate( radicand - numberRaisedByThePowerOfTheDegree ) < maximumDifferenceBetweenRootCalculatedRaisedByThePowerOfTheDegreeAndRadicand ), root )
