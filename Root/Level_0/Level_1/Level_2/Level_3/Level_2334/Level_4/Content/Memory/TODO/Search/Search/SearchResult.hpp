...
namespace Universe
{
    ...
    namespace GeneralPurposeContainers
    {
    	...
    	//  Purpose :
    	//
    	//     This object will be used to provide information to the user of the search results
    	//
   	    //  Description :
   	    //
  	    //
        ...
        public:
    		...
    		//		They will come
    		//
    		explicit inline SearchResult() throw();
            ...
            //      User information needed :
            //
            //          patternSize - the size of the pattern searched 
            //
            //      Information returned to the user :
            //
            //          NONE
            ...
            //      They will come
            //
            explicit inline SearchResult( UnsignedInteger64bits patternSize ) throw();
    		...
    		//		They will come
    		//
    		SearchResult( const SearchResult& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		SearchResult& operator=( const SearchResult& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		SearchResult( SearchResult&& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		SearchResult& operator=( SearchResult&& other ) throw( Exception );
    		...
    		//      3. 
    		//
    		bool operator==( const SearchResult& other ) const throw( Exception );
    	...
    	private:
            ...
            // This space will store the pattern size
            UnsignedInteger64bits m_patternSize;
...
//	****************************************************************   END PART OF DECLRATIONS, IMPLMENTATION TIME   ************************************************************
    	...
    	//		They will come
    	//
    	inline SearchResult::SearchResult() throw() :
        m_patternSize( 0 ),
        m_apperances(),
        m_overlapingPatternsInformation()
        {}
        ...
        //      They will come
        //
        inline SearchResult::SearchResult( UnsignedInteger64bits patternSize ) throw() :
    	...
    	//		They will come
    	//
    	SearchResult::SearchResult( const SearchResult& other ) throw( Exception ) :
        m_patternSize( other.m_patternSize ),
        m_apperances( other.apperances ),
        m_overlapingPatternsInformation( other.m_overlapingPatternsInformation )
        {}
    	...
    	//		They will come
    	//
    	SearchResult::SearchResult( SearchResult&& other ) throw( Exception ) :
        m_patternSize( other.m_patternSize ),
        m_apperances( other.apperances ),
        m_overlapingPatternsInformation( other.m_overlapingPatternsInformation )
    	{
    	...
    	//	    They will come
    	//
    	void SearchResult::SetToDefaultState() throw()
        {
            // Set the pattern size to 0
            m_patternSize = 0;

            // Set the array object containing the pattern to default state
            m_apperances.SetToDefaultState();

            // Set the array with the overlap patterns information to default state
            m_overlapingPatternsInformation.SetToDefaultState();            
        }
    	...
    	//		They will come
    	//
    	bool SearchResult::DefaultState() throw() const
    	{

    		// Check if the object is in default state
    		if ( ( m_patternSize == 0 ) &&
                 ( m_apperances.DefaultState() == true ) &&
                 ( m_overlapingPatternsInformation.DefaultState() ) )
    		{
    	...
    	//      2.
    	//
    	void SearchResult::DeleteResourcesAndSetToDefaultState() throw()
    	{
    		// Check if the object is in default state, then noting needs to be done
    		RETURN_IF_EXPRESSION_IS_TRUE( DefaultState() )

            // Set the size of the pattern to 0
            m_patternSize = 0;

    		// Release the resources of this object

            // Set the array object containing the pattern to default state and delete the resources
            m_apperances.DeleteResourcesAndSetToDefaultState();

            // Set the array with the overlap patterns information to default state and delete the resources
            m_overlapingPatternsInformation.DeleteResourcesAndSetToDefaultState(); 
    	}
        ...
        //  Description :
        //
        //      Use this method to add an apperance. The apperance will be added to end of the current apperances array
        //
        //  Required arguments :
        //
        //      User information needed :
        ...
        //      2.
        //
        void SearchResult::AddApperance( UnsignedInteger64bits apperance ) throw()
        {
            // Add the information to the end of the current apperances array
            m_apperances.PushTail( apperance );

            // Check if the current apperance overlap the previous
            if ( ( apperance - m_apperances.GetTail() ) < m_patternSize )
            {
                // The apperances overlap

                // When removing all the apperances
            }
        }
...
//  ***************************************************************  TEMPLATE OR INLINE OPERATORS IMPLEMENTATION   ************************************************
    	...
    	//      3. 
    	//
    	bool SearchResult::operator==( const SearchResult& other ) const throw( Exception )
        {

            // Check if the if the apperance and overlap information match
            if ( ( m_apperances != other.m_apperances ) || ( m_overlapingPatternsInformation != other.m_overlapingPatternsInformation ) )
            {
    	...
    	bool SearchResult::operator!=( const SearchResult& other ) const throw( Exception )
        {
            // Negate the result of the equal operator method and return it
            return ( !( *this == other ) );
        }
