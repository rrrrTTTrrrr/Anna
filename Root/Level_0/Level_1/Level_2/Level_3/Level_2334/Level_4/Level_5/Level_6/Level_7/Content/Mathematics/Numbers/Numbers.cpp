...
//  ********************************************************************   STATIC FUNCTIONS IMPLEMENTATION   ********************************************************************
		...
		//		They will come
		//
		Boolean Numbers::PrimalityTest( UnsignedInteger64bits number )
		{
		    // Each method that interacts with the user directly needs to ensure that the given data is valid
			Assert_MACRO( ( number != 0UL ), "The given number should not be 0" )
			...
			ReturnValueIfExpressionIsTrue_MACRO( ( ( ( number & 1 ) == 0UL ) || ( number == 1UL ) ), false )

			// Check if the number is 3 or 5, which are the two first prime numbers
			ReturnValueIfExpressionIsTrue_MACRO( ( ( number == 3UL ) || ( number == 5UL ) ), true )
			...
			for ( UnsignedInteger64bits order = 3UL ; order < divisor ; order += 2UL )
			{
				...
				ReturnValueIfExpressionIsTrue_MACRO( ( ( result * divisor ) == number ), false )
			}
		...
		UnsignedInteger64bits Numbers::LargestDivisorThatTheQuotientHasNoRemainder( UnsignedInteger64bits number )
		{
		    // Each method that interacts with the user directly needs to ensure that the given data is valid
			Assert_MACRO( ( number != 0UL ), "The given number should not be 0" )
			Assert_MACRO( ( PrimalityTest( number ) == false ), "The given number should not be prime" )
		...
		{		
			...
			// Check if the last bit of the number is ON or OFF, if the bit is OFF, this means that the number is even and it divides by 2 with no
			// remainder
			ReturnValueIfExpressionIsTrue_MACRO( ( ( number & 1 ) == 0UL ), 2UL )
		...
		{
		    // Each method that interacts with the user directly needs to ensure that the given data is valid
			Assert_MACRO( ( number1 != 0UL ), "The given first number should not be 0" )
			Assert_MACRO( ( number2 != 0UL ), "The given second number should not be 0" )
			Assert_MACRO( ( PrimalityTest( number1 ) == false ), "The given first number should not be prime" )
			Assert_MACRO( ( PrimalityTest( number2 ) == false ), "The given second number should not be prime" )
			Assert_MACRO( ( number1 != number2 ), "The given numbers should be different" )
			...
			// remainder
			ReturnValueIfExpressionIsTrue_MACRO( ( ( ( number1 & 1 ) == 0UL ) && ( ( number2 & 1 ) == 0UL ) ), true )

			// Check if one of the number is 2, and the other one does not divide by 2
			ReturnValueIfExpressionIsTrue_MACRO( ( ( ( number1 == 2UL ) || ( number2 == 2UL ) ) && ( ( ( number1 & 1 ) == 1UL ) || ( ( number2 & 1 ) == 1UL ) ) ), false )
			...
			for ( UnsignedInteger64bits smallerNumberDivisorsIndex = 0UL ; smallerNumberDivisorsIndex < smallerNumberDivisors.Size() ; ++smallerNumberDivisorsIndex )
			{
				...
				ReturnValueIfExpressionIsTrue_MACRO( ( ( largerNumber % smallerNumberDivisors[ smallerNumberDivisorsIndex ] ) == 0UL ), true )
			}
		...
		Memory::DynamicArray< UnsignedInteger64bits > Numbers::CommonDivisors( UnsignedInteger64bits number1, UnsignedInteger64bits number2 )
		{
			...
			Assert_MACRO( ( number2 != 0UL ), "The given second number should not be 0" )
			Assert_MACRO( ( number1 != number2 ), "The given numbers should be different" )
			Assert_MACRO( ( CommonDivisorTest( number1, number2 ) == true ), "The given numbers should have at least one common divisor" )
		...
		UnsignedInteger64bits Numbers::LargestCommonDivisor( UnsignedInteger64bits number1, UnsignedInteger64bits number2 )
		{
			...
			for ( UnsignedInteger64bits smallerNumberDivisorsIndex = ( smallerNumberDivisors.Size() - 1UL ) ; smallerNumberDivisorsIndex > 0UL  ; --smallerNumberDivisorsIndex )
			{
				...
				// copy of it
				ReturnValueIfExpressionIsTrue_MACRO( ( ( largerNumber % smallerNumberDivisors[ smallerNumberDivisorsIndex ] ) == 0UL ), smallerNumberDivisors[ smallerNumberDivisorsIndex ] )
			}
		...
		UnsignedInteger64bits Numbers::SmallestCommonMultiple( UnsignedInteger64bits number1, UnsignedInteger64bits number2 )
		{
			...
			// with no remainder
			ReturnValueIfExpressionIsTrue_MACRO( ( number1 == 1UL ), number2 )
			ReturnValueIfExpressionIsTrue_MACRO( ( number2 == 1UL ), number1 )

			// Check if both numbers are the same, then just return the number
			ReturnValueIfExpressionIsTrue_MACRO( ( number1 == number2 ), number1 )
			...
			if ( number1 < number2 )
			{
				...
				ReturnValueIfExpressionIsTrue_MACRO( ( ( number2 % number1 ) == 0UL ), number2 )
			...
			else
			{
				...
				ReturnValueIfExpressionIsTrue_MACRO( ( ( number1 % number2 ) == 0UL ), number1 )
			...
			while ( currentMultiplicationOfLargerNumber < numbersMultiplication )
			{
				...
				// of times
				ReturnValueIfExpressionIsTrue_MACRO( ( ( currentMultiplicationOfLargerNumber % smallerNumber ) == 0 ), currentMultiplicationOfLargerNumber )
...
// *********************************************************************************   TESTING   ********************************************************************************
		...
		DEBUG_TOOL(
			...
			void Numbers::Test()
			{
				Assert_MACRO( ( PrimalityTest( 17UL ) == true ), "Test failed - 1" )
				Assert_MACRO( ( PrimalityTest( 3UL ) == true ), "Test failed - 2" )
				Assert_MACRO( ( PrimalityTest( 8UL ) == false ), "Test failed - 3" )
				Assert_MACRO( ( PrimalityTest( 47UL ) == true ), "Test failed - 4" )

				Assert_MACRO( ( PrimalityTest( 9973UL ) == true ), "Test failed - 5" )
				Assert_MACRO( ( PrimalityTest( 9967UL ) == true ), "Test failed - 6" )
				Assert_MACRO( ( PrimalityTest( 9949UL ) == true ), "Test failed - 7" )
				Assert_MACRO( ( PrimalityTest( 9941UL ) == true ), "Test failed - 8" )
				Assert_MACRO( ( PrimalityTest( 9931UL ) == true ), "Test failed - 9" )
				Assert_MACRO( ( PrimalityTest( 9929UL ) == true ), "Test failed - 10" )
				Assert_MACRO( ( PrimalityTest( 9923UL ) == true ), "Test failed - 11" )
				Assert_MACRO( ( PrimalityTest( 9907UL ) == true ), "Test failed - 12" )
				Assert_MACRO( ( PrimalityTest( 9901UL ) == true ), "Test failed - 13" )
				Assert_MACRO( ( PrimalityTest( 9887UL ) == true ), "Test failed - 14" )
				Assert_MACRO( ( PrimalityTest( 9883UL ) == true ), "Test failed - 15" )
				Assert_MACRO( ( PrimalityTest( 9871UL ) == true ), "Test failed - 16" )

				Assert_MACRO( ( PrimalityTest( 9971UL ) == false ), "Test failed - 17" )
				Assert_MACRO( ( PrimalityTest( 9962UL ) == false ), "Test failed - 18" )
				Assert_MACRO( ( PrimalityTest( 9943UL ) == false ), "Test failed - 19" )
				Assert_MACRO( ( PrimalityTest( 9945UL ) == false ), "Test failed - 20" )
				Assert_MACRO( ( PrimalityTest( 9933UL ) == false ), "Test failed - 21" )
				Assert_MACRO( ( PrimalityTest( 9927UL ) == false ), "Test failed - 22" )
				Assert_MACRO( ( PrimalityTest( 9921UL ) == false ), "Test failed - 23" )
				Assert_MACRO( ( PrimalityTest( 9909UL ) == false ), "Test failed - 24" )
				Assert_MACRO( ( PrimalityTest( 9903UL ) == false ), "Test failed - 25" )
				Assert_MACRO( ( PrimalityTest( 9882UL ) == false ), "Test failed - 26" )
				Assert_MACRO( ( PrimalityTest( 9885UL ) == false ), "Test failed - 27" )
				Assert_MACRO( ( PrimalityTest( 9870UL ) == false ), "Test failed - 28" )
				...
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 8UL ) == 4UL ), "Test failed - 29" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 9UL ) == 3UL ), "Test failed - 30" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 12UL ) == 6UL ), "Test failed - 31" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 15UL ) == 5UL ), "Test failed - 32" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 144UL ) == 72UL ), "Test failed - 33" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 45UL ) == 15UL ), "Test failed - 34" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 90UL ) == 45UL ), "Test failed - 35" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 93UL ) == 31UL ), "Test failed - 36" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 12345UL ) == 4115UL ), "Test failed - 37" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 34567UL ) == 2659UL ), "Test failed - 38" )
				Assert_MACRO( ( LargestDivisorThatTheQuotientHasNoRemainder( 45797UL ) == 1117UL ), "Test failed - 39" )


				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 8UL ) == 2UL ), "Test failed - 40" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 16UL ) == 2UL ), "Test failed - 41" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 12464UL ) == 2UL ), "Test failed - 42" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 34565456342UL ) == 2UL ), "Test failed - 43" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 12222222222222UL ) == 2UL ), "Test failed - 44" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 22633365UL ) == 3UL ), "Test failed - 45" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 69002381UL ) == 7UL ), "Test failed - 46" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 9805939UL ) == 11UL ), "Test failed - 47" )
				Assert_MACRO( ( SmallestDivisorThatTheQuotientHasNoRemainder( 12294625131UL ) == 3UL ), "Test failed - 48" )


				auto testAllDivisors_1 = MOVE( AllDivisorsThatTheQuotientHasNoRemainder( 24 ) );

				Assert_MACRO( ( testAllDivisors_1[0] == 2UL ), "Test failed - 49" )
				Assert_MACRO( ( testAllDivisors_1[1] == 3UL ), "Test failed - 50" )
				Assert_MACRO( ( testAllDivisors_1[2] == 4UL ), "Test failed - 51" )
				Assert_MACRO( ( testAllDivisors_1[3] == 6UL ), "Test failed - 52" )
				Assert_MACRO( ( testAllDivisors_1[4] == 8UL ), "Test failed - 53" )
				Assert_MACRO( ( testAllDivisors_1[5] == 12UL ), "Test failed - 54" )


				auto testAllDivisors_2 = MOVE( AllDivisorsThatTheQuotientHasNoRemainder( 40 ) );

				Assert_MACRO( ( testAllDivisors_2[0] == 2UL ), "Test failed - 55" )
				Assert_MACRO( ( testAllDivisors_2[1] == 4UL ), "Test failed - 56" )
				Assert_MACRO( ( testAllDivisors_2[2] == 5UL ), "Test failed - 57" )
				Assert_MACRO( ( testAllDivisors_2[3] == 8UL ), "Test failed - 58" )
				Assert_MACRO( ( testAllDivisors_2[4] == 10UL ), "Test failed - 59" )
				Assert_MACRO( ( testAllDivisors_2[5] == 20UL ), "Test failed - 60" )


				auto testAllDivisors_3 = MOVE( AllDivisorsThatTheQuotientHasNoRemainder( 123 ) );

				Assert_MACRO( ( testAllDivisors_3[0] == 3UL ), "Test failed - 61" )
				Assert_MACRO( ( testAllDivisors_3[1] == 41UL ), "Test failed - 62" )


				auto testAllDivisors_4 = MOVE( AllDivisorsThatTheQuotientHasNoRemainder( 11324 ) );
				
				Assert_MACRO( ( testAllDivisors_4[0] == 2UL ), "Test failed - 63" )
				Assert_MACRO( ( testAllDivisors_4[1] == 4UL ), "Test failed - 64" )
				Assert_MACRO( ( testAllDivisors_4[2] == 19UL ), "Test failed - 65" )
				Assert_MACRO( ( testAllDivisors_4[3] == 38UL ), "Test failed - 66" )
				Assert_MACRO( ( testAllDivisors_4[4] == 76UL ), "Test failed - 67" )
				Assert_MACRO( ( testAllDivisors_4[5] == 149UL ), "Test failed - 68" )
				Assert_MACRO( ( testAllDivisors_4[6] == 298UL ), "Test failed - 69" )
				Assert_MACRO( ( testAllDivisors_4[7] == 596UL ), "Test failed - 70" )
				Assert_MACRO( ( testAllDivisors_4[8] == 2831UL ), "Test failed - 71" )
				Assert_MACRO( ( testAllDivisors_4[9] == 5662UL ), "Test failed - 72" )
				...
				{
					Assert_MACRO( ( CommonDivisorTest( number * multiplier1, number * multiplier2 ) == true ), "Test failed - 73" )
				}
				...
				for ( UnsignedInteger64bits number = 3UL ; number < 534UL ; ++number )
				{
					Assert_MACRO( ( CommonDivisorTest( number * multiplier1 * multiplier3, number * multiplier2 * multiplier3 ) == true ), "Test failed - 74" )

					multiplier3 -= 77UL;
				}

				Assert_MACRO( ( PrimalityTest( 5UL ) == true ), "Test failed - 75" )
				Assert_MACRO( ( CommonDivisorTest( 2UL, 87UL ) == false ), "Test failed - 76" )
