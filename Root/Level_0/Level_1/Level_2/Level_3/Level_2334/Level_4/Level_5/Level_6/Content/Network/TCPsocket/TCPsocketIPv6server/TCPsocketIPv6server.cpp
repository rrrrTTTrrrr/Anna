#ifndef  TCPSOCKETIPV6SERVER_CPP
#define  TCPSOCKETIPV6SERVER_CPP


#include "TCPsocketIPv6server.hpp"
...
//  ***************************************************************   PUBLIC METHODS IMPLEMENTATION - INTERFACE   ***************************************************************
    ...
    //      They will come
    //
    void TCPsocketIPv6server::Delete() throw()
    {
    ...
    //      2. 
    //
    void TCPsocketIPv6server::Initiate( std::string serverIPv6address, int serverPortNumber, unsigned int flowInfo = 0, unsigned int scopeID = 0 ) throw( ErrorHandling::ErrnoException )
    {
        ...
        // Bind the port number and the IPv6 address in the socket
        BindIMP();
        ...
        m_pollAndAcceptConnections = std::thread( &TCPsocketIPv6server::PollAndAcceptConnectionsIMP, this );
    }
...
//  ********************************************************************   PRIVATE METHODS IMPLEMENTATION   *********************************************************************
    ...
    //      2.
    //
    inline void TCPsocketIPv6server::ListenIMP() throw( ErrorHandling::ErrnoException )
    {
        ...
        if ( listen( m_socketFileDescriptor, -1 ) == 0 )
        {
            ...
            throw( ErrorHandling::ErrnoException( "TCPsocketIPv6server::ListenIMP - the listen call failed" ) );
        }
    ...
    //      2.
    //
    inline int TCPsocketIPv6server::AcceptIMP() throw( ErrorHandling::ErrnoException )
    {
        ...
        // This object will store the length of the IPv6 socket address structure
        unsigned int structureLength = sizeof( m_IPv6sockaddr );

        // Accept the connection
        newConnection = accept( m_socketFileDescriptor, reinterpret_cast< struct sockaddr* >( &m_IPv6sockaddr ), &structureLength );

        // Check the accept return status
        if ( newConnection == -1 )
        {
            // The accept call failed, throw an exception describing the problem
            throw( ErrorHandling::ErrnoException( "TCPsocketIPv6server::ListenIMP - the accept call failed" ) );
        }
    ...
    //      2.
    //
    inline void TCPsocketIPv6server::PollAndAcceptConnectionsIMP() throw( ErrorHandling::ErrnoException )
    {
        ...
        while ( m_keepRunning == true )
        {
            ...
            if ( pollReturnStatus == -1 )
            {
                ...
                if ( errno != EINTR )
                {
                    ...
                    throw( ErrorHandling::ErrnoException( "TCPsocketIPv6server::PollAndDispatchIMP - the poll call failed" ) );
                }
    ...
    //      2.
    //
    std::vector< int > TCPsocketIPv6server::PollOpenConnectionsIMP() throw( ErrorHandling::ErrnoException )
    {
        ...
        while ( m_keepRunning == true )
        {
            ...
            if ( pollReturnStatus == -1 )
            {
                ...
                {
                    // The poll call failed, thrown an exception describing the error
                    throw( ErrorHandling::ErrnoException( "TCPsocketIPv6server::PollOpenConnectionsIMP - the poll call failed" ) );
                }
        ...
        for ( unsigned long openConnectionsIndex = 0 ; openConnectionsIndex < m_openConnections.size() ; ++openConnectionsIndex )
        {
            ...
            else if ( ( ( m_openConnections[ openConnectionsIndex ].GetReturnedEvents() & POLLERR ) == true ) && ( ( m_openConnections[ openConnectionsIndex ].GetReturnedEvents() & POLLNVAL ) == true ) )
            {
                ...
                throw( ErrorHandling::ErrnoException( "TCPsocketIPv6server::PollOpenConnectionsIMP - the connection had error" ) );
            }
