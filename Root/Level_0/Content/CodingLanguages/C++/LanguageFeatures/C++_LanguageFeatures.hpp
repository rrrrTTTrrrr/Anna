#ifndef CPP_LANGUAGEFEATURES_HPP
#define CPP_LANGUAGEFEATURES_HPP


#include "../../../Preprocessor/Preprocessor_Features.hpp"  //  All preprocessor basic definitions
...
// ************************************************************************************************************************************************************************** //
	...
	using DoublePrecisionFloatingPoint = double;
...
// ************************************************************************************************************************************************************************** //
	...
	#define DisableCopyConstructorAndAssignmentOperator_MACRO( ObjectName ) 		\
																					\
	...
	#define DisableMoveConstructorAndMoveAssignmentOperatorForTemplateObject_MACRO( ObjectName ) 		\
																										\
	...
	#define DisableCopyAndMove_MACRO( ObjectName ) 													\
																									\
		/* Disable the copy constructor and assignment operator	*/									\
		DisableCopyConstructorAndAssignmentOperator_MACRO( ObjectName )								\
																									\
		/* Disable the move constructor and move assignment operator	*/							\
		DisableMoveConstructorAndMoveAssignmentOperatorForTemplateObject_MACRO( ObjectName )		\
	...
	#define NoInstances_MACRO( ObjectName ) 									\
																				\
		...
		DisableCopyAndMove_MACRO( ObjectName )									\



// ************************************************************************************************************************************************************************** //
// **														   				 OBJECT EQUAL OPERATOR PROLOGUE    												   			   ** //
// ************************************************************************************************************************************************************************** //



	// This macro will be used inside the equal operator of each object, to add the functionality to deal with instances in default state
	#define CompareInstances_BothInDefaultStateReturnTrue_OnlyOneInDefaultStateReturnFalse_OtherwiseContinue_MACRO														\
																																										\
																																										\
		/* Check if both instances are in default state */																												\
		if ( ( member_state.Get() == InstanceState::Default ) && ( other.State().Get() == InstanceState::Default ) )													\
		{																																								\
			/* Both instances in default state, return true */																											\
			return ( true );																																			\
		}																																								\
		/* Check if only one of the instances is in default state */																									\
		else if ( ( member_state.Get() == InstanceState::Default ) || ( other.State().Get() == InstanceState::Default ) )												\
		{																																								\
			/* Only one instance is in default state, retrun false */																									\
			return ( false );																																			\
		}
	...
	// This macro will be used inside the equal operator of each object, to add the functionality to deal with instances in default state
	#define CompareInstances_TakeStateFromMember_BothInDefaultStateReturnTrue_OnlyOneInDefaultStateReturnFalse_OtherwiseContinue_MACRO( MemberName )					\
																																										\
																																										\
		/* Check if both instances are in default state */																												\
		if ( ( MemberName.State().Get() == InstanceState::Default ) && ( other.MemberName.State().Get() == InstanceState::Default ) )									\
		{																																								\
			/* Both instances in default state, return true */																											\
			return ( true );																																			\
		}																																								\
		/* Check if only one of the instances is in default state */																									\
		else if ( ( MemberName.State().Get() == InstanceState::Default ) || ( other.MemberName.State().Get() == InstanceState::Default ) )								\
		{																																								\
			/* Only one instance is in default state, retrun false */																									\
			return ( false );																																			\
		}
...
// ************************************************************************************************************************************************************************** //
	...
	#define ReturnNullIfPointerIsNull_MACRO( Pointer ) if ( ( Pointer == nullptr ) ) { return ( nullptr ); }


	// The current macro is defined as a macro and not as an inline function becuase the return statement won't have the right affect in an inline function, the required 
	// operation is not to return to the diredt caller of this macro, but to the caller of the the function called this macro
	#define ReturnIfPointerIsNull_MACRO( Pointer ) if ( ( Pointer == nullptr ) ) { return; }
...
// ************************************************************************************************************************************************************************** //
	...
	#define ReturnErrorNumberIfCommandLineArgumentsAreNotEnough_MACRO( CommandLineArgumentsNumber, RequiredCommandLineArguments, ErrorNumber ) if ( ( CommandLineArgumentsNumber < RequiredCommandLineArguments ) ) { return ( ErrorNumber ); }


	// Use this to check that the number of command line arguments is equal the required amount
	#define  ReturnErrorNumberIfCommandLineArgumentsAreNotEqual_MACRO ( CommandLineArgumentsNumber, RequiredCommandLineArguments, ErrorNumber ) if ( ( CommandLineArgumentsNumber != RequiredCommandLineArguments ) ) { return ( ErrorNumber ); }


	// Use this to check that the number of command line arguments is equal the required amount
	#define DisplayErrorMessaeAndReturnErrorNumberIfCommandLineArgumentsAreNotEqual_MACRO( CommandLineArgumentsNumber, RequiredCommandLineArguments, ErrorNumber, ErrorMessage ) if ( ( CommandLineArgumentsNumber != RequiredCommandLineArguments ) ) { std::cout << ErrorMessage << std::endl; return ( ErrorNumber ); }
...
// ************************************************************************************************************************************************************************** //
	...
	#define ReturnValueIfExpressionIsTrue_MACRO( Expression, ReturnValue ) if( ( ( Expression ) == true ) ) { return ( ReturnValue ); }
	...
	// with inline functions it is immposible
	#define ReturnValueIfExpressionIsFalse_MACRO( Expression, ReturnValue ) if( ( ( Expression ) == false ) ) { return ( ReturnValue ); }
	...
	#define ReturnIfExpressionIsTrue_MACRO( Expression ) if( ( ( Expression ) == true ) ) { return; }


	// Use this to check the result of an expression is true then return. it is useful because from this return the current function is ended, while  with inline 
	// functions it is immposible
	#define ReturnIfExpressionIsFalse_MACRO( Expression ) if( ( ( Expression ) == false ) ) { return; }
	...
	// it is immposible
	#define ReturnIfTheValuesAreEqual_MACRO( VALUE_1, VALUE_2 ) if( ( VALUE_1 == VALUE_2 ) ) { return; }
	...
	// it is immposible
	#define ReturnIfTheValuesAreNotEqual_MACRO( VALUE_1, VALUE_2 ) if( ( VALUE_1 != VALUE_2 ) ) { return; }
...
// ************************************************************************************************************************************************************************** //
	...
	// Verify if the current encode is little or big endian, returns true for little endian and false for big endian  
...
// ************************************************************************************************************************************************************************** //
	...
	// This function will be used to delete object created using displacment new. All it needs is the address and the type of the object. The destructor must not take 
	// any arguments
	template< typename Type >
	void DisplacmentDeleteForPointers( void* addressOfObjectToDelete )
	{
		// Cast the pointer to the given object type, and call the object destructor
		reinterpret_cast< Type* >( addressOfObjectToDelete )->~Type();
	}
	...
	// any arguments
	template< typename Type >
	void DisplacmentDeleteForRefernces( Type& objectToDelete )
	{
...
// *****************************************************************************   Specifiers   ***************************************************************************** //
	...
	//   			   call operator will be constant expression anyway, if it happens to satisfy all constant exprssion function requirements
...
// ************************************************************************************************************************************************************************** //
	...
	constexpr UnsignedInteger64bits KiloByte( UnsignedInteger64bits SizeInKiloByte )
	{
	...
	constexpr UnsignedInteger64bits MegaByte( UnsignedInteger64bits SizeInMegaByte )
	{
	...
	constexpr UnsignedInteger64bits GigaByte( UnsignedInteger64bits SizeInGigaByte )
	{
		// To convert between MegaBytes to single bytes the following method is used :
		//
...
// ************************************************************************************************************************************************************************** //
	...
	// This object will passed to constructor of objects, to generate a random instance during construction
	class RandomInstance
	{};

	inline RandomInstance GenerateRandomInstance;



// ************************************************************************************************************************************************************************** //
// **														   			  	  TEMPLATE PACKS SEPERATOR                                                                     ** //
// ************************************************************************************************************************************************************************** //



	class TemplatePacksSeperator
	{};
...
// ************************************************************************************************************************************************************************** //
	...
	/* Align type with specific ALIGN_TO boundary */
	#define ATTRIBUTE_ALIGN_TO( ALIGN_TO ) __attribute__ ( ( aligned ( ALIGN_TO ) ) )

	/* Align to the largest alignment which is ever used for any data type on the target machine */
	#define ATTRIBUTE_ALIGNED() __attribute__ ( ( aligned ) )

	/* Set the new location on the screen to be printed on */
...
#endif   // CPP_LANGUAGEFEATURES_HPP
