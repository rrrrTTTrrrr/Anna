#ifndef ASSEMBLYBASICUTILITIES_HPP
#define ASSEMBLYBASICUTILITIES_HPP
...
// ************************************************************************************************************************************************************************** //
	...
	//		
	#define AddToAssemblyInstructionLineEndMarkAndSurroundWithQuationMarks_MACRO( TEXT ) "\"" #TEXT ";"
...
// ***************************************************************   ERROR - operand type mismatch for `cmp'   *************************************************************** //
	...
	// When using the 'CMP' assembly instruction in in the following way :
	//
	//		CMP 0xFFFFF000, EAX;
	//
...
// ************************************************************************************************************************************************************************** //
	...
	// 		radix options :
	//
	// 			h - hexadecimal ( hexadecimal constnat beginning with a letter must have a leading zero to prevent the assmbler from interpreting it as an identifier - 
	//				instead of A3h its 0A3h )
	// 			q/o - octal
	// 			d - decimal
	// 			b - binary
	//
	// 		if no radix is given the integer constnat is assumed to be decimal.
	...
	// Strings :
	//
	// 	An ASCII character and string can be enclosed with one or double quotes
	//
	...
	// Interrupts :
	//
	// 	int - means in assembly interrupt



	// General layout of an NASM source file :
	//
	//
	//		SECTION .data
	//
	...
	//		SECTION .text
	//
	//		global main     ;make main available to operating system
	//
	//		main:
	//
	...
	//		ret
	//
	// From our code template above, creation and destruction of our function main's stack frame is achieved by saving the callers ebp, creating a new ebp by assigning ebp 
	// a new value. Then we destroy the frame by restoring esp to where it was prior to main being code. Then finally by popping the value of the callers base pointer back 
	// into ebp
	//
	// So really, the only way to declare local variables is by utilizing the stack. The stack ( stack frame ) is nothing than a dynamic memory abstraction.  Its got two 
	// components, "esp" ( the stack pointer ) and "ebp" ( the base pointer ). The esp is always changing while ebp is "created" and destroyed by moving the current value 
	// of esp into and out of it



	// Creating local variables in assembly :
	//
	// Our goal here is to create a local variable inside of the main method. The only way to create a local variable is by using the stack. 
	//
	// Why?  
	//
	// Because we can only declare variable in storage locations and the only available storage locations are text, bss, and data. However, text section is only for code, 
	// so it is out of the question. The bss and data sections are appealing, but to declare our "local" variable in these sections will defeat the purpose of these 
	// variables being local, they would be global.
	//
	...
	//
	//     		;we use increment of 4 because check push is 4 bytes in length because the stack is structured to hold 32 bit(4byte) values notably address, which are 
	//			;4 bytes in length
...
#endif   // ASSEMBLYBASICUTILITIES_HPP
