#ifndef  READWRITETEXTFILE_H
#define  READWRITETEXTFILE_H
...
namespace Universe
{
    ...
    namespace
    {
        ...
        public:
    		...
    		//		They will come
    		//
    		explicit inline ReadWriteTextFile() throw();
    		...
    		//		They will come
    		//
    		explicit ReadWriteTextFile() throw( Exception );
    		...
    		//		They will come
    		//
    		ReadWriteTextFile( const ReadWriteTextFile& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		ReadWriteTextFile& operator=( const ReadWriteTextFile& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		ReadWriteTextFile( ReadWriteTextFile&& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		ReadWriteTextFile& operator=( ReadWriteTextFile&& other ) throw( Exception );
    		...
    		//		They will come
    		//
    		inline ~ReadWriteTextFile() throw();
    		...
    		//  Description :
    		//
    		//      Use this operator to check if the this object matches the other object. Important note - if only 1 object
    		//      is in default state and the other is not, the method returns false, but if both objects are in default state,
    		//      then they actually match but contain nothing, this is taken as an error and an exception is thrown
    		//
    		//  Required arguments :
    		//
    		//      User information needed :
    		...
    		//      3. 
    		//
    		bool operator==( const ReadWriteTextFile& other ) const throw( Exception );
    		...
    		//  Description :
    		//
    		//      Use this operator to check if the this object does not match the other object. Important note - if only 1 object
    		//      is in default state and the other is not, the method returns true, but if both objects are in default state,
    		//      then they actually match but contain nothing, this is taken as an error and an exception is thrown
    		//
    		//  Required arguments :
    		//
    	...
    	private:
            ...
            Encoding::UnicodeDoubleLinkedList m_file;

            // This object will be used to download the file into secondary memory
            DownloadFileToSecondaryMemory m_downloadFileToSecondaryMemory;
...
//	****************************************************************   END PART OF DECLRATIONS, IMPLMENTATION TIME   ************************************************************
    	...
    	//		They will come
    	//
    	inline ReadWriteTextFile::ReadWriteTextFile() throw() :
        m_uploadFileToMainMemory(),
        m_encoding( Encoding::Unknown ),
        m_file(),
        m_downloadFileToSecondaryMemory()
        {}
    	...
    	//		They will come
    	//
    	ReadWriteTextFile::ReadWriteTextFile() throw( Exception ) :
        m_uploadFileToMainMemory( fileNameIncludingFullPath ),
        m_encoding( Encoding::Unknown ),
        m_file(),
        m_downloadFileToSecondaryMemory( fileNameIncludingFullPath )
        {
            // Create an object to store the address of the file in maim memory
            DesignPatterns::SmartPointers::ExceptionSafePointer< UnsignedInteger8bits > addressOfFileInMainMemory( m_uploadFileToMainMemory.UploadTheEntireFile(), DesignPatterns::SmartPointers::ARRAY_OF_OBJECTS );
            ...
            ThrowExceptionIfExpressionIsTrue( ( SmartPointers::Unknown == m_encoding ), "ReadWriteTextFile::Constructor failed - the encoding of the file could not been resolved" );

            // Store the file inside the unicode double linked list
    	...
    	//		They will come
    	//
    	ReadWriteTextFile::ReadWriteTextFile( const ReadWriteTextFile& other ) throw( Exception ) :
    	...
    	//		They will come
    	//
    	ReadWriteTextFile::ReadWriteTextFile( ReadWriteTextFile&& other ) throw( Exception ) :
    	{
    	...
    	//	    They will come
    	//
    	void ReadWriteTextFile::SetToDefaultState() throw()
    	...
    	//		They will come
    	//
    	bool ReadWriteTextFile::DefaultState() throw() const
    	{
    	...
    	//      2.
    	//
    	void ReadWriteTextFile::DeleteResourcesAndSetToDefaultState() throw()
    	{
...
//  ***********************************************************  TEMPLATE OR INLINE OPERATORS IMPLEMENTATION   *******************************************
    	...
    	//      3. 
    	//
    	bool ReadWriteTextFile::operator==( const ReadWriteTextFile& other ) const throw( Exception )
    	{
    		// Check that the 2 object are not in default state
    		ThrowExceptionIfExpressionIsTrue( ( ( true == DefaultState() ) && ( true == other.DefaultState() ) ), "ReadWriteTextFile::operator== failed - the 2 objects are in default state" );
    	}
