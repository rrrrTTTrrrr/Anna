	...
	*/
	class PHPcookie
	{
        ...
        public function __construct( $name, $value = NULL, $expirationTime = 0, $accessOnlyPath = NULL, $accessOnlyDomain = NULL, $secure = false, $httpOnly = false ) 
        {
            ...
            // Set a new cookie
            if ( setcookie( $name, $value, ( time() + $expirationTime ), $accessOnlyPath, $accessOnlyDomain, $secure, $httpOnly ) === false )
            {
              // Store the back trace
              $backTrace = debug_backtrace();

              // Display to the user the error message
              trigger_error( 'Failed creating cookie : ' . $name . ' with ' . $value . ' in ' . $backTrace[0][ 'file' ] . ' on line ' . $backTrace[0][ 'line' ], E_USER_NOTICE );
