...
inline IndexBuffer< AnyType >::IndexBuffer() :
...
template< typename AnyType >
inline IndexBuffer< AnyType >::IndexBuffer( const UnsignedInteger32bits* const data, const UnsignedInteger32bits size ) :
    member_rendererId( NumericLimits< UnsignedInteger32bits >::Maximum ),
    member_size( size )
{
    ...
    // Bind a named buffer object. The target 'GL_ELEMENT_ARRAY_BUFFER' is to describe the required vertices to use. Binds a buffer
    // object to the specified bufer binding point
    glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, member_rendererId );

    // Create and initialize a buffer object's data store. The buffer object currently bound to the target 'GL_ELEMENT_ARRAY_BUFFER'
    ...
    //
    glBufferData( GL_ELEMENT_ARRAY_BUFFER, size * sizeof( UnsignedInteger32bits ), data, GL_STATIC_DRAW );
}
...
template< typename AnyType >
inline void IndexBuffer< AnyType >::Bind() const
{
...
template< typename AnyType >
inline void IndexBuffer< AnyType >::Unbind() const
{
...
DEBUG_TOOL(
	...
	template< typename AnyType >
	void IndexBuffer< AnyType >::UnitTest()
	{}
